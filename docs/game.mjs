...
import {frameAt,frameViewBox} from './runner.mjs';
...
try {
  ...
  const player=el('g',{class:'paused',id:'player'});
  player.append(el('ellipse',{rx:11,ry:3,cy:5,class:'runner-shadow'}));
  const facing=el('g',{id:'runner-facing'});
  const sprite=el('svg',{x:-27,y:-66,width:54,height:72,viewBox:frameViewBox(0),overflow:'hidden',id:'runner-frame'});
  sprite.append(el('image',{href:'assets/run-cycle.png',width:1536,height:1024}));
  facing.append(sprite);player.append(facing);board.append(player);
  let strideStart=0;
  ...
  async function travel(a,b,version){
    if(b.x!==a.x)facing.setAttribute('transform',b.x<a.x?'scale(-1 1)':'scale(1 1)');
    if(reduced){moveTo(b);return;}
    await new Promise(resolve=>{let begin;function tick(time){if(version!==generation){resolve();return;}begin??=time;const distance=Math.hypot(b.x-a.x,b.y-a.y);const duration=Math.max(180,distance*9);const t=Math.min(1,(time-begin)/duration);sprite.setAttribute('viewBox',frameViewBox(frameAt(time-strideStart)));moveTo({x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t});if(t<1)requestAnimationFrame(tick);else resolve();}requestAnimationFrame(tick);});
  }
  async function roll(){
    if(busy||position>=route.length-1)return;
    busy=true;strideStart=performance.now();$('roll').disabled=true;player.classList.remove('paused');
    ...
    player.classList.add('paused');sprite.setAttribute('viewBox',frameViewBox(0));busy=false;
